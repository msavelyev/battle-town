...
const data = require('@Cljs/code/tanks.lib.data.js');
...
function move(world, id, direction, updates) {
  ...
  if (!tank || tank.state === EntityState.DEAD) {
    return data.modified_unsuccessfully(world);
  }
  ...
  if (tank) {
    ...
    return data.modified_successfully(world);
  } else {
    return data.modified_unsuccessfully(world);
  }
}
  ...
  for (let netMessage of unackedMessages) {
    switch (netMessage.type) {
      case MessageType.MOVE:
        ...
        world = data.get_result(move(world, id, tankMove.direction, []));
        break;
