...
export function update(match, event, beforeWorld, onKill) {
  ...
  let world = beforeWorld(match, event, updates);

  world = World.update(world, event, (killer, victim) => {
    return onKill(match, event, killer, victim, updates);
  }, updates);
...
function handleMove(match, netMessage, method, updates) {
  ...
  match.world = method(match.world, id, tankMove.direction, updates);
...
export function sync(match, id, tickData) {
  ...
  match.world = World.sync(match.world, tickData.updates, id, match.unackedMessages);
}
...
export function transitionState(match, event, updates) {
  switch (match.state) {
    ...
    case MatchState.state.SCORE:
      match.world = World.resetLevel(match.world, level.generate(level.choose()));
...
export function addUser(match, user, tick, updates) {
  ...
  match.world = World.newTank(match.world, tick, user, updates);
}
...
export function resetLevel(match, blocks) {
  match.world = World.resetLevel(match.world, blocks);
}
