...
import Entity, {DEAD_TICKS, REVIVING_TICKS} from './Entity.js';
...
import TankAdd from './worldevent/TankAdd.js';
...
export default class World {
  ...
  static reviveTanks(world, event, updates) {
    World.updateEntitiesState(
      ...
      tank => {
        ...
        World.createTank(world, tank);
        const newTank = World.findTank(world, tank.id);
  ...
  static updateEntitiesState(world, entities, event, fromState, ticksThreshold, onUpdate) {
    for (let entity of entities) {
      ...
      if (ticksDiff > ticksThreshold && World.collides(world, entity).length === 0) {
        onUpdate(entity);
      }
    }
  }
