...
export default class World {
  ...
  static reviveTanks(world, event, updates) {
    World.reviveEntities(world, world.tanks, event, updates, TankUpdate.fromTank);
  }

  static reviveBlocks(world, event, updates) {
    World.reviveEntities(world, world.blocks, event, updates, BlockUpdate.fromBlock);
  }

  static reviveEntities(world, entities, event, updates, updateFn) {
    for (let entity of entities) {
      ...
      if (ticksDiff > REVIVING_TICKS && World.collides(world, entity).length === 0) {
        Entity.makeAlive(entity);
        updates.push(updateFn(entity));
      }
    }
  }
