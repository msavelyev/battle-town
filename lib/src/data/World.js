...
import BlockRemove from './worldevent/BlockRemove.js';
...
import TankRemove from './worldevent/TankRemove.js';
...
export default class World {
      ...
      if (collisions.length > 0) {
        for (let collision of collisions) {
          if (collision.entityType === EntityType.TANK) {
            World.killTank(world, bullet, collision, onKill, updates);
          } else if (collision.entityType === EntityType.BLOCK) {
            World.removeBlock(world, collision, updates);
          }
        }
  ...
  static killTank(world, bullet, tank, onKill, updates) {
    World.removeTank(world, tank.id);
    updates.push(new TankRemove(tank.id));
  ...
  static removeBlock(world, block, updates) {
    if (block.type !== BlockType.BRICK) {
      return;
    }

    world.blocks = world.blocks.filter(b => b !== block);
    updates.push(new BlockRemove(block.id));
  }
